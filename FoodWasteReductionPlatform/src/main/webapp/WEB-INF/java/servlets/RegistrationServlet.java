...
public class RegistrationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String userType = request.getParameter("userType").toUpperCase(); // Convert to uppercase
        String joinDate = request.getParameter("joinDate");

        LOGGER.info("UserType received: " + userType);

        UserDTO user = new UserDTO();
        user.setUserId(UUID.randomUUID().toString()); // Generate a unique ID
        ...
        if (isUserAdded) {
            response.sendRedirect("index.jsp"); // Redirect to index page after successful registration
        } else {
